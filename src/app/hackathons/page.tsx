  ...
  return (
    <div className="max-w-4xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
      <div className="mb-6 sm:mb-8 flex items-center justify-between">
        <div>
          <p className="text-sm text-[var(--muted)] mb-1">{'>'} hackathon_module</p>
          <h1 className="text-3xl sm:text-4xl font-bold glow">Hackathons_</h1>
        </div>
        {user && (
          <Link href="/hackathons/host" className="btn-primary text-base py-2.5 sm:py-3 px-4 sm:px-6">
            [+ host hackathon]
          </Link>
        )}
      </div>
      ...
      {loading ? (
        <div className="text-center py-8">
          <p className="text-base text-[var(--muted)]">{'>'} loading hackathons...</p>
        </div>
      ) : (
        <div className="space-y-4">
          {hackathons.length === 0 ? (
            <p className="text-base text-[var(--muted)]">No hackathons listed yet.</p>
          ) : (
            hackathons.map((hackathon) => (
              <div key={hackathon.id} className="card p-5 sm:p-6">
                <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-4">
                  <div className="flex-1">
                    <h2 className="text-xl sm:text-2xl font-bold mb-1 glow-subtle">
                      {hackathon.title}
                    </h2>
                    <p className="text-base sm:text-lg text-[var(--foreground-dim)] mb-2 sm:mb-3">{hackathon.description}</p>
                  ...
                  {user && (
                    <button
                      ...
                      className={`${hackathon.joined ? 'btn-secondary' : 'btn-primary'} text-base py-2 px-5 w-full sm:w-auto`}
                    >
