...
export default function IdeaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
      <div className="mb-4">
        <Link href="/collab" className="text-sm sm:text-base text-[var(--muted)] hover:text-[var(--foreground-dim)] transition-colors">
          ← back to ideas
        </Link>
      </div>
      ...
      >
        <header className="mb-6">
          <h1 className="text-3xl sm:text-4xl font-bold glow mb-3">
            {idea.title}
          </h1>
          <p className="text-base sm:text-lg text-[var(--foreground-dim)] mb-4">
            {idea.description}
          </p>
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-[var(--muted)] uppercase tracking-wider mb-1">Problem</p>
            <p className="text-base sm:text-lg text-[var(--foreground-dim)]">{idea.problem}</p>
          </div>
        </div>

        <div className="mb-6 p-4 sm:p-5 border border-[var(--border)] rounded bg-[var(--card-bg)]/50">
          <p className="text-sm font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Solution</p>
          <p className="text-base sm:text-lg text-[var(--foreground-dim)]">{idea.solution}</p>
        </div>

        {idea.sketch_url && (
          <div className="mb-6">
            <p className="text-sm font-bold text-[var(--muted)] uppercase tracking-wider mb-2">Sketch / Draft</p>
            <div className="border border-[var(--border)] rounded p-4 sm:p-6 bg-[var(--card-bg)]/30 flex items-center justify-center min-h-[200px]">
              <div className="text-center">
                ...
                <p className="text-base text-[var(--muted)]">Sketch: {idea.sketch_url}</p>
              </div>
            </div>
          </div>
        )}
